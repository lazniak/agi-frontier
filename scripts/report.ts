...
for (const m of rows) {
  ...
  console.log(`  ${m.date} ${m.lab.padEnd(9)} ${r.name.padEnd(28)} ${m.index.toFixed(1).padStart(5)} ± ${m.se.toFixed(2)}  n=${m.n}`);
}

console.log('\nRanking of current flagships:');
rankCurrentFlagships(fit, releases, asOf).forEach((m, i) => {
  const r = releases.find((x) => x.id === m.release_id)!;
  console.log(`  ${String(i + 1).padStart(2)}. ${r.name.padEnd(28)} ${m.lab.padEnd(9)} ${m.index.toFixed(1)}  cov=${(m.coverage * 100).toFixed(0)}%`);
});
