...
| T30 | Redesign · shared maths I: Frontier Rating, weighted Rasch, Elo→p, generations, levels ladder, frontier trend + fan, level crossings, pace eras (REDESIGN §1–2, §9) | shared | in-progress | glm-worker |
| T31 | Redesign · shared maths II: lineup bands, tier filter, infinite forecast/fan, backtest & calibration (REDESIGN §3–5, §9) | shared | in-progress | glm-worker |
| T32 | Redesign · worker: researcher v2 — backfill, arena, eval, promote; retry/backoff + concurrency; state/progress fields; weekly loop steps (REDESIGN §6) | worker | in-progress | glm-worker |
...
| T35 | Redesign · docs: PAPER.md (the standard, in the user's name), METHODOLOGY/DATA-GUIDE/README updates | docs | in-progress | main-session |
